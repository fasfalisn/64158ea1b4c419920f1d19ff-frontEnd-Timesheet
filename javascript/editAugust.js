let apiAugustApi = new TempApi.AugustApi(); import TempApi from '../src/index'; let apiUserApi = new TempApi.UserApi(); document.getElementById('icy1c').onclick = (event) => {
  ...
  { history.back(); }
}; document.addEventListener('alignemployee', function (e) {
  ...
  let augustId = window.location.pathname.replace('/editAugust/', ''); apiAugustApi.getaugust(augustId, (error, data, response) => {
  ...
  }); apiUserApi.getAlluser((error, data, response) => {
    if (error) { console.error(error); } else {
      console.log('API called successfully. Returned data: ' + data); const subDataElements = [...document.getElementById("iz4vf").querySelectorAll("[dataitem='true']")].filter(
        (element, index, array) =>
