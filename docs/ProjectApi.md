...
[**getByParamsproject**](ProjectApi.md#getByParamsproject) | **GET** /project/getByParams | Get all the data based on user query
...
> [Project] getByParamsproject(filter)
...
apiInstance.getByParamsproject(filter, (error, data, response) => {
  if (error) {
    console.error(error);
  } else {
    console.log('API called successfully. Returned data: ' + data);
